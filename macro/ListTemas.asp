...
if request("M")="E" then
	...
	Conn.open constr
	conn.execute("delete TemaFuncion where cod_tema="& request("icodtema"))
	conn.close
	set conn=nothing
end if
...
conn.open constr
	for key = 1 to request("hdni")
		if request("chkactivo"&key)="1" then
			sql="Update TemaFuncion set est_activo='1' where cod_tema="&request("hdncodigo"&key)
		else
			sql="Update TemaFuncion set est_activo='0' where cod_tema="&request("hdncodigo"&key)
		end if
		conn.execute(sql)
	next
conn.close
...
<%'Listado
	...
	StrSQl="Select cod_tema,nom_tema,est_Activo from TemaFuncion order by 2 asc"
...
fActivo=trim(rs(2))
  ...
  <tr class="texto">
    <td align="left"><%=sNomTema%></td>
      <td align="center"><a href="EditTemas.asp?M=M&icodtema=<%=icodtema%>" class="texto" onMouseover="showtip(this,event,'Modifica datos de <%=replace(ucase(trim(sNomTema)),"'","\'")%>')" onMouseout="hidetip()">Modificar</a></td>
      <td align="center"><a href="JavaScript:fConfirmAction('ListFunciones.asp?M=E&icodtema=<%=icodtema%>', 'Desea Eliminar el registro?')" class="texto" onMouseover="showtip(this,event,'Elimina los datos del tema <%=replace(ucase(trim(sNomTema)),"'","\'")& factivo%> ')" onMouseout="hidetip()" >Borrar</a></td>
	  <td align="center"><input type="checkbox" name="chkActivo<%=i%>" value="1" <%if factivo="1" then%>checked <%end if%>>
		   <input type="hidden" name="hdnCodigo<%=i%>" value="<%=icodtema%>">
	   </td>
  </tr>
  <%
...
<%else%>
			<tr bgcolor="#FFFFFF">
				<td valign="middle" align="center" class="texto" colspan="5">
					Por el momento no existen temas registrados
				</td>
			</tr>
<%end if
'	Rs.Close
'	set Rs= nothing%>
			<tr bgcolor="#FFFFFF">
				<td valign="top" colspan="<%if i>0 then%>3<%else%>6<%end if%>">
					<a href="EditTemas.asp?M=N" class="texto">Nuevo tema </a>
				</td>
<%if i>0 then%>
