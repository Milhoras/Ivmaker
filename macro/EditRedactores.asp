...
case "M"
	...
	conn.open Constr
	strsql="SELECT cod_usuario, nom_usuario, des_email,des_usuario,des_clave, est_Activo " & _
...
case "G"							'Grabar
	...
	'icodNivel  =request("cbmNivel")
	if fActivo="" then fActivo=0
	if sNomUsuario="" or len(sNomUsuario)>50  or  sUsuario=""  or  sClave=""  or  sEmail="" then
		...
		Set conn = server.CreateObject("adodb.Connection")
		Conn.open Constr
		strsql="Insert Usuario (Nom_usuario, des_email,des_usuario,des_clave,est_Activo) " & _
			   " values('"&sNomUsuario&"','"&sEmail&"','"&sUsuario&"','"&sClave&"','"&fActivo&"')"
        conn.execute(strsql)
        set codRs = conn.execute("select @@ identity from usuario")
        icodusuario=CodRs(0)
        strsql="Insert Usuarioperfil (cod_usuario,cod_publicacion)" & _
				" values ("&icodusuario&","&icodpublicacion&")"
		conn.close
		set conn=nothing
...
case "U"
	...
	if factivo ="" then factivo=0
	if sNomUsuario="" or len(sNomUsuario)>50  or len(sUsuario)>15  or len(sClave)>15  or  sUsuario=""  or  sClave=""  or  sEmail=""    then
		...
		strsql="Update  Usuario set Nom_Usuario='"&snomUsuario&"' , des_email='"&sEmail&"'," & _
			   " des_usuario='"&susuario&"', des_clave='"&sclave&"', est_activo='"&factivo&"'" & _
...
<form action="<%=request.Servervariables("url")%>" method="post">
  <TABLE cellpadding="1" cellspacing="1" border="0" width="480" align="center">
    ...
    <tr>
      ...
      <td class="texto">
        <input type="text" name="txtNomUsuario" value="<%=sNomUsuario%>">
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td class="texto">
        <input type="text" name="txtEmail" value="<%=sEmail%>">
      </td>
    <tr>
      <td colspan="5" class="LineaSeparadora"></td></tr>
	 ...
	 <tr>
      ...
      <td class="texto">
        <input type="text" name="txtusuario" value="<%=sUsuario%>">
      </td>
    </tr>
    ...
    <tr>
      ...
      <td class="texto">
        <input type="text" name="txtClave" value="<%=sClave%>">
      </td>
    </tr>
...
<%end if%>
    ...
    <tr align="center">
      <Td colspan="2" class="Titulo">
        ...
        <input type="button" name="accion" value="Volver a Lista" onclick="location.href='ListRedactores.asp?icodpublicacion=<%=icodpublicacion%>'">
      </td>
    </tr>
  </table>
</form>
